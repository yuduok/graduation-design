"""
Stanford Cars 实验结果汇总脚本
Collect Stanford Cars experiment results and generate comparison tables/plots
"""
import os
import re
import json
...
def collect_results(base_dir):
    ...
    if not experiments:
        ...
        print("  bash run_experiments_stanford_cars.sh cuda")
        return {}
...
def print_comparison_table(results):
    ...
    # 终端表格
    print("\n" + "=" * 80)
    print("  Stanford Cars Classification: Top-1 Accuracy (%)")
...
def print_latex_table(results):
    ...
    print("\\begin{table}[h]")
    print("\\centering")
    print("\\caption{Stanford Cars 分类准确率对比 (自适应 Epoch 策略)}")
    print("\\label{tab:stanford_cars_comparison}")
...
def plot_learning_curves(base_dir, results):
    ...
    ax.set_title("Stanford Cars Fine-Grained Classification: Method Comparison", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description="Collect Stanford Cars experiment results")
    parser.add_argument(
        "--base-dir",
        type=str,
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_fgd", "stanford_cars"),
    ...
    print("Collecting Stanford Cars experiment results...")
