...
针对上述问题,本文提出一种面向细粒度猫狗分类的动态提示词优化方法 DynamicPromptTrainer。该方法以 CLIP 为基础,在提示学习框架中加入三个关键部件: 一是 AdaptivePromptLearner,在基础上下文向量外叠加类别自适应因子; 二是 SoftPromptAdapter,利用两层 MLP 根据图像特征生成提示词偏移; 三是 DifficultyWeightCalculator,根据模型对真实类别的预测置信度和错误情况生成可学习的样本权重,使训练过程对困难样本给予更高关注。与此同时,本文采用随 shot 数变化的自适应训练轮数策略,以缓解低样本条件下训练不足和高样本条件下过拟合的问题。
...
This thesis proposes a dynamic prompt optimization method for fine-grained cat and dog classification, named DynamicPromptTrainer. Built on CLIP, the method combines three components: AdaptivePromptLearner for class-adaptive context scaling, SoftPromptAdapter for generating image-conditioned prompt offsets through a two-layer MLP, and DifficultyWeightCalculator for assigning learnable weights according to prediction confidence and misclassification status. In addition, an adaptive epoch schedule is adopted so that lower-shot settings receive longer training while higher-shot settings avoid unnecessary overfitting.
