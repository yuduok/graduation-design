...
围绕上述目标,本文设计并实现了 DynamicPromptTrainer。系统仍以 CLIP 为主干,视觉编码器和文本编码器保持冻结,训练阶段只更新提示学习相关模块。这样做的原因很直接: Oxford-IIIT Pets 的 few-shot 训练样本有限,如果直接微调整个主干,参数量过大,训练稳定性也更难控制。把更新范围限制在提示部分,既能保留 CLIP 的跨模态表示能力,也便于和 CoOp、CoCoOp 做对比。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{Chinese-English-architecture.png}
\caption{DynamicPromptTrainer 系统架构}
...
AdaptivePromptLearner 是提示学习部分的入口,对应项目中的 \texttt{models/dynamic\_prompt.py}。该模块先初始化一组可学习上下文向量 \(\mathbf{c}_1,\mathbf{c}_2,\ldots,\mathbf{c}_{n_{ctx}}\)。初始化模板采用 ``a photo of a'',在当前实现中实际对应 4 个上下文 token。这样做并不追求把模板写得多复杂,而是先给提示一个稳定起点,后续再通过训练去调整。

与 CoOp 只维护一组共享上下文不同,本文在基础上下文之外加入类别自适应因子 \(\boldsymbol{\alpha}\)。对于第 \(i\) 个上下文向量,其类别相关表示写为
\begin{equation}
...
如果只有 AdaptivePromptLearner,系统虽然能学习上下文,但同一类别的不同图像仍然共享近似相同的提示表达。为了解决这个问题,本文加入 SoftPromptAdapter。该模块是一个两层 MLP,输入为视觉编码器输出的图像特征 \(\mathbf{f}_v \in \mathbb{R}^{d}\),输出为与上下文维度一致的偏移向量 \(\Delta \mathbf{c}\)。其计算形式为
\begin{equation}
...
这一模块和 CoCoOp 的基本思路接近,都是让提示跟着图像走,而不是保持静态。不同之处在于,本文没有把问题停留在“提示如何变化”上,还进一步关心“训练时哪些样本更需要被强调”。因此 SoftPromptAdapter 负责输入适配,而样本难度判断交给后面的 DifficultyWeightCalculator。

\subsection{DifficultyWeightCalculator}

DifficultyWeightCalculator 用于为每个训练样本生成权重。这里没有直接把损失值当成难度,而是根据模型对真实类别的预测概率以及预测是否正确来估计样本难度。这样做更贴近训练过程本身,因为模型真正“拿不准”的样本,通常就体现在真实类别概率偏低或直接分错上。
